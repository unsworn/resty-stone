...
function keystoneOnMount(){
    ...
    if (rs.keystone.session.disableCookies === true) {
        ...
        // TODO: Tempoary hack to handle Express 3 and 4 internal differences. 
        var routesStack = rs.keystone.app.stack ? rs.keystone.app.stack : rs.keystone.app._router.stack;

        var sessionOptions = rs.keystone.get('session options');
        for(var i=0; i < routesStack.length; i++){
            if (routesStack.handle === sessionOptions.cookieParser) {
                ...
                originalSessionMiddleware = routesStack[1].handle
                ...
                routesStack.splice(i, 2, routesStack.pop()); // remove 2, cookieParser & express.session
            }
        }
    }
}
